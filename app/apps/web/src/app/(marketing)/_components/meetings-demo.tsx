 ...
 * in chunk by chunk (the way Deepgram lands finals), then Elevay extracts the
 * action items and buying signals — ready to review.
 */
...
const PHASES = [0, 1100, 2600, 4200, 5800, 7400]; // join · rec · c1 · c2/c3 · actions · signals
const CYCLE_MS = 9500;
...
export function MeetingsDemo() {
  ...
  const chunksShown = phase <= 2 ? Math.max(0, phase - 1) : phase <= 3 ? CHUNKS.length : CHUNKS.length;
  const showActions = phase >= 4;
  const showSignals = phase >= 5;

  return (
    <div ref={ref}>
      <ScaleToFit designWidth={1080}>
        <AppFrame url="app.elevay.com/meetings">
          <div className="grid grid-cols-[1fr_320px]" style={{ height: 560, background: T.page }}>
            {/* ── Call + transcript ── */}
            <div className="flex flex-col p-5">
              ...
              <div className="mt-4 flex-1 space-y-2.5 overflow-hidden rounded-xl border p-4" style={{ borderColor: T.border, background: T.card }}>
            ...
            {/* ── Extracted: action items + buying signals ── */}
            <div className="flex flex-col gap-4 border-l p-5" style={{ borderColor: T.border, background: T.card }}>
